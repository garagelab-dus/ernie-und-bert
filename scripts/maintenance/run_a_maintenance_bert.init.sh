...
# This script is for maintenance purposes to read motor data from BERT 
# (a lerobot so-101 leader arm) in case of issues.
# It connects to the BERT, reads various registers from all motors, and displays them in a table. 
# It also has an optional "check after reassembly" mode 
# to move motors back to their original positions after maintenance.
#
# If you want to maintenance ERNIE, use run_a_maintenance_ernie.init.sh instead.
...
# check if the following variables are set: bert_type, bert_port, bert_id
if [ -z "$bert_type" ]; then
  echo "Error: bert_type is not set"
  exit 1
fi 
if [ -z "$bert_port" ]; then
  echo "Error: bert_port is not set"
  exit 1
fi
if [ -z "$bert_id" ]; then
  echo "Error: bert_id is not set"
  exit 1
fi

# print the variables
echo "bert_type: $bert_type"
echo "bert_port: $bert_port"
echo "bert_id: $bert_id"
echo "debug: $debug"
...
bert_type = os.environ["bert_type"]
bert_port = os.environ["bert_port"]
bert_id = os.environ["bert_id"]
...
if bert_type not in supported_types:
  print(
    f"Error: maintenance motor read currently supports {sorted(supported_types)} only; got '{bert_type}'",
    file=sys.stderr,
  )
  sys.exit(1)

teleop = SOLeader(SOLeaderTeleopConfig(port=bert_port, id=bert_id))
...
def init_debug_log():
  ...
  return logs_dir / f"maintenance_{bert_id}_{timestamp}.log"


if debug_enabled:
  log_path = init_debug_log()
  with log_path.open("a", encoding="utf-8") as f:
    f.write(f"[{datetime.now().isoformat()}] DEBUG enabled for {bert_id} on {bert_port}\n")
...
def write_exception_log(exc, label, log_file_path=None):
  if log_file_path is None:
    ...
    logs_dir = Path("maintenance") / "logs"
    logs_dir.mkdir(parents=True, exist_ok=True)
    log_file_path = logs_dir / f"maintenance_error_{bert_id}_{timestamp}.log"
...
try:
  ...
  if Table is not None and Console is not None:
    console = Console()
    console.print(make_rich_table(rows, "Bert Motor Readout (Snapshot)"))
  else:
    print_plain_table(rows, "Bert Motor Readout (Snapshot)")
  ...
  if Table is not None and Console is not None and Live is not None and interactive:
    ...
    with cbreak_mode(tty_input):
      with Live(
        make_rich_table(
          ...
          "Bert Motor Readout (Live, press Q to quit)",
        ),
        console=console,
        refresh_per_second=10,
      ) as live:
    ...
    if not interactive:
      ...
      print_plain_table(
        collect_rows(bus, unresponsive_motor_names),
        "Bert Motor Readout (Second Snapshot)",
      )
    else:
      with cbreak_mode(tty_input):
        while True:
